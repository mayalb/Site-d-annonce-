...
function calcRoute(){
    ...
    function(result,status){
     if(status == google.maps.DirectionsStatus.OK){
        ...
        var re = /[0-9]+(   )?/g;
            distance.value=result.routes[0].legs[0].distance.text.replace(new RegExp("[^(0-9)]", "g"), '') ; 
            aff.innerText=result.routes[0].legs[0].distance.text;
         directionsDisplay.setDirections(result);
         
     }else{
         distance.value="0"; 
         aff.innerText="Distance non disponible";
